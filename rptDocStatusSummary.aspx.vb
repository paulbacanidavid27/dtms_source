...
Public Class rptDocStatusSummary
    ...
    Private Function DeriveTitle() As String
        If Request.Cookies("udrDocStatStatus").Value = "O" Then
            Return "Open Documents / " & Request.Cookies("udrDocStatActionTakenDesc").Value & " " & "Last Action"
        ElseIf Request.Cookies("udrDocStatStatus").Value = "C" Then
            Return "Completed/Closed Documents /" & Request.Cookies("udrDocStatActionTakenDesc").Value & " " & "Last Action"
        Else
            Return "All " & "Status / " & Request.Cookies("udrDocStatActionTakenDesc").Value & " " & "Last Action"
        End If
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '" case when dl.statusid = 12 then 'Completed/Closed' when dl.statusid = 18 then 'Completed/Closed' when dl.statusid = 15 then 'Completed/Closed' when dl.statusid = 8 then 'Completed/Closed' else 'Open' end as statusdesc, "
        '" case when dr.statusid = 12 then 'Completed/Closed' when ds.Description = 'No Action Required' then 'Completed/Closed' when dr.statusid = 14 then 'Completed/Closed' when dr.statusid = 8 then 'Completed/Closed' when dr.statusid = 15 then 'Partially Completed' when dr.statusid = 16 then 'On-Hold' else 'Open' end as statusdesc, "
        ...
        If DocSession.OraClient Then
            s_sql = s_sql & "(NVL(u2.FirstName,u.FirstName)||' '||NVL(u2.LastName,u.LastName)) AS assignedto, " & _
                        ...
                        "NVL(TO_CHAR(dr.actiondate,'mm/dd/yyyy'),' ') AS adate, " & _
        ...
        Else
            ...
            s_sql = s_sql & "(isnull(u2.FirstName,u.LastName)+' '+isnull(u2.LastName,u.LastName)) AS assignedto, " & _
                       "isnull(convert(varchar(10),dr.actiondate,101),'') AS adate, " & _
                       ...
                       "case when isdate(div.colvalue)=1 then div.colvalue else convert(varchar(10),dl.createddate,101) end as cdate," & _
                        "datediff(d,case when isdate(div.colvalue)=1 then convert(datetime,div.colvalue) else dl.createddate end,isnull(dr.actiondate,getdate())) AS age, " & _
        ...
        If Request.Cookies("udrDocStatRecvFrom").Value <> "" Then
            If DocSession.OraClient Then
                s_sql = s_sql & " AND TRUNC(dl.createddate) >= TO_DATE('" & Request.Cookies("udrDocStatRecvFrom").Value & "','mm/dd/yyyy')"
            Else
        ...
        If Request.Cookies("udrDocStatRecvTo").Value <> "" Then
            If DocSession.OraClient Then
                s_sql = s_sql & " AND trunc(dl.createddate) <= TO_DATE('" & Request.Cookies("udrDocStatRecvTo").Value & "','mm/dd/yyyy')"
            Else
