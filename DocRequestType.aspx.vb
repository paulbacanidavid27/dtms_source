...
Public Class DocRequestType
    ...
    Private Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        UserControlAdminMenuH1.SelectTab("Request")
...
#End Region
    ...
    Private Sub RetrieveRequestType()
        ...
        Try
            ...
            Dim oDoc As New docRequest
            'oDoc.pGroupID = GroupCode.Text.Trim
            oDoc.pRequestDescription = tbSearchDesc.Text.Trim
            ...
            If ldata.Rows.Count > 0 Then
                ...
                Dim lstotalrows As String = oDoc.CountDocRequest
    ...
    Protected Sub btSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btSave.Click
        If tbDesc.Text.Trim = "" Then
            ...
            msg.Text = "** Request Type is a required field."
        Else
    ...
    Private Function ReceiptDoesNotExist() As Boolean
        ...
        Try
            ...
            objCommand.CommandText = "SELECT d.RequestType,d.RequestDescription FROM docRequestType d WHERE d.RequestDescription = '" & tbDesc.Text & "'"
            'objCommand.ParametersAddWithValue("@groupid", tbGroupCode.Text)

            If objCommand.ExecHasRow Then
                msg.CssClass = "msg_red"
                msg.Text = "Request Type " & tbDesc.Text & " already exist. Please try another description."
                Return False
            Else
                Return True
            End If
    ...
    Private Sub SaveDocReceipt()
        ...
        Dim loDocGroup As docRequest
        Dim lbAdd As Boolean
        Try
            ec = New crypt
            loDocGroup = New docRequest
            lbAdd = False
            objCommand = New clsSqlConn(ltr.pTran)

            objCommand.CommandType = CommandType.Text
            loDocGroup.pRequestType = DocSession.getNextID("DocRequest")
            loDocGroup.pRequestDescription = tbDesc.Text
            loDocGroup.pOffice = dlOffice.SelectedValue
            loDocGroup.pUserId = DocSession.sUserId
            loDocGroup.pIPAddress = Request.UserHostAddress
            loDocGroup.pAgeCalc = dlAgeCalc.SelectedValue
            loDocGroup.AddRequest(objCommand)
            ...
            If lAction.Text.ToLower = "add" Then
                ...
                msg.Text = "** Request Type " & tbDesc.Text.Trim & " has been created successfully."
            Else
                msg.CssClass = "msg_green"
                msg.Text = "** Request Type " & tbDesc.Text.Trim & " has been updated successfully."
            End If
    ...
    Private Sub UpdateDocReceipt()
        ...
        Try
            ...
            loDocGroup.pRequestType = ltReceiptId.Text
            loDocGroup.pAgeCalc = dlAgeCalc.SelectedValue
            loDocGroup.pRequestDescription = tbDesc.Text
            loDocGroup.pOffice = dlOffice.SelectedValue
    ...
    Private Sub UpdateReceipt(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        Dim ltlag As Literal = DirectCast(ri.FindControl("lAgeCalc"), Literal)
        Dim loff As Literal = DirectCast(ri.FindControl("lOffice"), Literal)
        ...
        dlAgeCalc.SelectedValue = ltlag.Text
        dlOffice.SelectedValue = loff.Text
    ...
    Private Sub DeleteReceipt()
        ...
        Try
            ...
            loData.Columns.Add("RequestType", Type.GetType("System.String"))
            loData.Columns.Add("RequestDescription", Type.GetType("System.String"))
            ...
            For Each ri In Repeater1.Items
                If ri.ItemType = ListItemType.Item Or ri.ItemType = ListItemType.AlternatingItem Then
                    ...
                    If cbox.Checked Then
                        loRow = loData.NewRow()
                        loRow("RequestType") = DirectCast(ri.FindControl("lReceiptID"), Literal).Text
                        loRow("RequestDescription") = DirectCast(ri.FindControl("lReceiptDesc"), Literal).Text
                        loRow("Comment") = IIf(oDoc.CheckIfRequestExists(loRow("RequestType")), "Cannot delete record being used.", "OK to delete.")
    ...
    Protected Sub btDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btDelete.Click
        ...
        Try
            ...
            Dim odg As New docRequest

            For Each ri In Repeater2.Items
                If ri.ItemType = ListItemType.Item Or ri.ItemType = ListItemType.AlternatingItem Then
                    If DirectCast(ri.FindControl("lComment"), Literal).Text = "OK to delete." Then

                        odg.pRequestType = DirectCast(ri.FindControl("lId"), Literal).Text
                        odg.pRequestDescription = DirectCast(ri.FindControl("lDesc"), Literal).Text
