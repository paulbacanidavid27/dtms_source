Public Class Site
    ...
    Public WriteOnly Property ShowImageDocument() As Boolean
        ...
        End Set
    End Property
    Public Sub ShowModal(asValue As Boolean)

        cfBoxLayout.Visible = asValue
        cfBoxLayout2.Visible = asValue
    ...
    Public Sub SelectTab(ByVal asTab As String)
        If asTab = "Account" OrElse asTab = "Search" OrElse asTab = "Import" OrElse asTab = "Monitoring" OrElse asTab = "Tracking" OrElse asTab = "Holiday" Then
            setGreen()
        Else
            setBlue()
        End If
        userControlTab1.Visible = True
            ...
            If Not IsPostBack Then
                ...
                If lsTO.Trim <> "" AndAlso IsNumeric(lsTO) Then
                    ...
                    If DocSession.sUserName <> "" Then
                        ...
                        If DocSession.sUserRole <> "H" AndAlso DocSession.sUserRole <> "B" Then
                            pSearch.Visible = True
                        Else
                            pSearch.Visible = False
                            If DocSession.sUserRole = "B" Then
                                hlQuickGuide.Visible = False
    ...
    Protected Sub imgTopSearch_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgTopSearch.Click
        If tbTopSearch.Text.Trim = "" OrElse tbTopSearch.Text.Trim = "Enter search criteria here ..." Then
            ...
            searchoption.Update()
        Else
            imgTopSearch.ToolTip = "Search"
            DocSession.SearchCriteria = tbTopSearch.Text.Trim
            'SetParam()
            DocSession.srchAS = "Q"
            DocSession.srchASCurrentPage = "1"
            'Dim mycookie = New HttpCookie("srchCurrent")
            'mycookie.Value = "1"
            'mycookie.Expires = DateTime.Now.AddDays(30)
            'Response.Cookies.Add(mycookie)

            'mycookie = New HttpCookie("srchAS")
            'mycookie.Value = "2"
            'mycookie.Expires = DateTime.Now.AddDays(30)
            'Response.Cookies.Add(mycookie)
            Response.Redirect("search.aspx?as=2") '& Server.UrlEncode(tbTopSearch.Text))
            'Server.Transfer("search.aspx?p=" & Server.UrlEncode(tbTopSearch.Text))
        End If
    ...
    End Sub
    Public Sub setBlue()
        matt.Style("background-color") = "#DDF0FA"
    End Sub
