Public Class ucReceiptRoute
    ...
    Public Property pCreatedDate As String
        ...
        End Get
        Set(ByVal value As String)
            lackDate.Text = CDate(value).ToString("MM/dd/yyyy")
            lUploadDate.Text = value
        End Set
    End Property
    ...
    'Private Sub btPrintRoute_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btPrintRoute.Click
    '    Dim oDoc As DocList
    '    Try
    '        oDoc = New DocList
...
"GroupLogo = isnull(g.GroupLogo,''), " &
        "AddressDesc = isnull(oa.AddressDesc,''), " &
        "ReceiptReplyName = isnull(g.ReceiptReplyName,''), " &
        "OfcCode = isnull(g.OfficeCode,''), " &
        "GroupId = isnull(g.GroupId,'') " &
"from Users u inner join Groups g " &
"on u.UserGroup = g.GroupId " &
"inner join Office o " &
"on o.OfficeCode = g.OfficeCode " &
"inner join OfficeAddress oa " &
