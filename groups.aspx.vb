...
#End Region
        ...
        For Each ri In Repeater3.Items
            If ri.ItemType = ListItemType.Item Or ri.ItemType = ListItemType.AlternatingItem Then
                ...
                If DirectCast(ri.FindControl("ImgSelected"), ImageButton).Visible Then
                    ...
                    Dim rbAccess As RadioButtonList = DirectCast(ri.FindControl("rbGroupAccess"), RadioButtonList)
                        Dim iAllow As Boolean = DirectCast(ri.FindControl("ImgAllow"), ImageButton).Visible
                    'objCommand.ParametersClear()
                    'objCommand.ParametersAddWithValue("@GroupId", tbGroupCode.Text)
                    'objCommand.ParametersAddWithValue("@DocId", DirectCast(ri.FindControl("DocType"), Literal).Text)
                    'objCommand.ParametersAddWithValue("@GroupAccessId", CInt(rbAccess.SelectedValue))
                    'objCommand.ParametersAddWithValue("@GroupAccessDesc", rbAccess.SelectedItem.Text)
                    ...
                    'objCommand.ParametersAddWithValue("@CanPrint", IIf(DirectCast(ri.FindControl("ImgAllow"), ImageButton).Visible, 1, 0))
                    'objCommand.ParametersAddWithValue("@CanDownload", IIf(DirectCast(ri.FindControl("ImgDownload"), ImageButton).Visible, 1, 0))
                    'objCommand.ParametersAddWithValue("@VersionControl", IIf(DirectCast(ri.FindControl("ImgVersionAllow"), ImageButton).Visible, 1, 0))
                        loDocGroup.pGroupID = tbGroupCode.Text

                        loDocGroup.pGroupAccessId = rbAccess.SelectedValue
                        loDocGroup.pGroupAccessDesc = rbAccess.SelectedItem.Text
                        loDocGroup.pDocType = DirectCast(ri.FindControl("DocType"), Literal).Text

                        loDocGroup.pUserId = DocSession.sUserId

                        loDocGroup.pCanPrint = IIf(iAllow, "1", "0")
                        loDocGroup.pCanDownLoad = IIf(DirectCast(ri.FindControl("ImgDownload"), ImageButton).Visible, "1", "0")
                        loDocGroup.pCanPrintReceipt = IIf(DirectCast(ri.FindControl("ImgReceipt"), ImageButton).Visible, "1", "0")
                        loDocGroup.pVersionControl = IIf(DirectCast(ri.FindControl("ImgVersionAllow"), ImageButton).Visible, "1", "0")
                        loDocGroup.pDocTypeDesc = DirectCast(ri.FindControl("DocName"), Literal).Text
                        loDocGroup.pIPAddress = Request.UserHostAddress
                        loDocGroup.AddGroupDocAccess(objCommand)
    ...
    Private Sub CopyAccess(ByVal objCommand As clsSqlConn)
        ...
        Try
            ...
            loDocGroup.CopyGroupDocAccess(objCommand, tbGroupCode.Text)
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    End Sub
    ...
    Private Sub UpdateGroup()
        ...
        Try
            ...
            'objCommand.ParametersClear()
            'objCommand.ParametersAddWithValue("@GroupId", tbGroupCode.Text)
            'objCommand.ParametersAddWithValue("@GroupName", tbGroupName.Text)
            'objCommand.ParametersAddWithValue("@GroupNameOrig", lGroupNameOrig.Text)
            'objCommand.ParametersAddWithValue("@UserId", DocSession.sUserId)
            'objCommand.ParametersAddWithValue("@IPAddress", Request.UserHostAddress)
            'objCommand.ParametersAddWithValue("@ReportAccess", IIf(cbReportAccess.Checked, 1, 0))
            'objCommand.ExecTranNonQuery()

            'objCommand.CommandType = CommandType.StoredProcedure
            'objCommand.CommandText = "xMSP_GROUPDOCADD"
            loDocGroup.pGroupID = tbGroupCode.Text
            loDocGroup.pDesc = tbGroupName.Text
            ...
            loDocGroup.DeleteGroupDocAccess(objCommand)
            AddAccess(objCommand)
    ...
    Private Sub UpdateGroup(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        Try
            ...
            ldata = oGroup.RetrieveGroupAccess()
    ...
    Private Sub Repeater1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles Repeater1.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            End If
            DirectCast(e.Item.FindControl("ucWorkSched"), UserControlWorkSched).GroupId = DirectCast(e.Item.FindControl("GroupCode"), Literal).Text
            DirectCast(e.Item.FindControl("ucDocAccess"), UserControlDocAccess).GroupId = DirectCast(e.Item.FindControl("GroupCode"), Literal).Text
    ...
    Public Sub fSelectDocType(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        For Each rptItems As RepeaterItem In Repeater3.Items
            DirectCast(rptItems.FindControl("imgSelect"), ImageButton).Visible = Not DirectCast(rptItems.FindControl("imgSelect"), ImageButton).Visible
            DirectCast(rptItems.FindControl("imgSelected"), ImageButton).Visible = Not DirectCast(rptItems.FindControl("imgSelected"), ImageButton).Visible
            Dim updPnl As UpdatePanel = DirectCast(rptItems.FindControl("pnlGAccess"), UpdatePanel)
            Dim rbList As RadioButtonList = DirectCast(rptItems.FindControl("rbGroupAccess"), RadioButtonList)
            If rbList.SelectedValue = "" Then
                rbList.SelectedValue = "0"
                rbList.DataSource = oAccess.RetrieveGroupAccess()
                rbList.DataTextField = "GroupAccess"
                ...
                rbList.DataBind()
            End If
            If DirectCast(rptItems.FindControl("imgSelected"), ImageButton).Visible Then
                updPnl.Visible = True
                rbList.Visible = True
                updPnl.Update()
            Else
    ...
    Private Sub Repeater3_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles Repeater3.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Dim ltPrintAllowed As Literal = DirectCast(e.Item.FindControl("GCanPrint"), Literal)
            Dim ltDownloadAllowed As Literal = DirectCast(e.Item.FindControl("GCanDownload"), Literal)
            Dim ltVersionAllowed As Literal = DirectCast(e.Item.FindControl("GVersionControl"), Literal)
            Dim ltReceipt As Literal = DirectCast(e.Item.FindControl("GReceipt"), Literal)
            'Dim cbx As CheckBox = DirectCast(e.Item.FindControl("cbxSelect"), CheckBox)
            Dim Img As ImageButton = DirectCast(e.Item.FindControl("ImgSelect"), ImageButton)
            Dim ImgS As ImageButton = DirectCast(e.Item.FindControl("ImgSelected"), ImageButton)
            Dim ImgNAp As ImageButton = DirectCast(e.Item.FindControl("ImgNotAllow"), ImageButton)
            Dim ImgAp As ImageButton = DirectCast(e.Item.FindControl("ImgAllow"), ImageButton)
            Dim ImgND As ImageButton = DirectCast(e.Item.FindControl("ImgNotDownload"), ImageButton)
            Dim ImgD As ImageButton = DirectCast(e.Item.FindControl("ImgDownload"), ImageButton)
            Dim ImgVNAp As ImageButton = DirectCast(e.Item.FindControl("ImgVersionNotAllow"), ImageButton)
            Dim ImgVAp As ImageButton = DirectCast(e.Item.FindControl("ImgVersionAllow"), ImageButton)
            Dim ImgNR As ImageButton = DirectCast(e.Item.FindControl("ImgNotReceipt"), ImageButton)
            Dim ImgR As ImageButton = DirectCast(e.Item.FindControl("ImgReceipt"), ImageButton)
            If ltReceipt.Text = "Yes" Then
                ImgNR.Visible = False
                ImgR.Visible = True
            End If
            If ltPrintAllowed.Text = "Yes" Then
                ImgNAp.Visible = False
                ImgAp.Visible = True
            End If
            If ltVersionAllowed.Text = "Yes" Then
                ImgVNAp.Visible = False
                ImgVAp.Visible = True
            End If
            If ltDownloadAllowed.Text = "Yes" Then
                ImgND.Visible = False
                ImgD.Visible = True
            End If
            ...
            If ltlDT.Text.Trim <> "" Then
                ImgS.Visible = True
                Img.Visible = False
                Dim Rbl As RadioButtonList = DirectCast(e.Item.FindControl("rbGroupAccess"), RadioButtonList)
                Dim pnl As UpdatePanel = DirectCast(e.Item.FindControl("pnlGAccess"), UpdatePanel)
                Dim lDocAccess As Literal = DirectCast(e.Item.FindControl("GDocAccess"), Literal)
                Dim oAccess As New DocGroupAccess

                Rbl.Visible = True
                Rbl.DataSource = oAccess.RetrieveGroupAccess
                Rbl.DataValueField = "GroupAccessId"
                Rbl.DataTextField = "GroupAccess"

                Rbl.DataBind()

                Rbl.SelectedValue = CInt(lDocAccess.Text)
                pnl.Visible = True
                pnl.Update()
            End If
        End If
    ...
    Protected Sub imgShowWorkSched(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim userDocAccess As UserControlDocAccess
        userDocAccess = DirectCast(oItem.FindControl("ucDocAccess"), UserControlDocAccess)
